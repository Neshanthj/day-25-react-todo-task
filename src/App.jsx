...
function App() {
  ...
  const addTodo = () => {
    ...
    setID(id + 1);
    document.querySelector('.input-container input[type="text"]:nth-child(1)').value = ''; // Reset name input to an empty string
    document.querySelector('.input-container input[type="text"]:nth-child(2)').value = ''; // Reset description input to an empty string
};
    ...
    <div className="App">
      ...
      <div className="filter-container mt-5">
        ...
        <div className="status-filter">
          ...
          <select
            className={`selection rounded p-1 ${
              filter === 'completed'
                    ...
                    <div className="col mb-5">
                      <div key={task.number} className="card p-3">
                        ...
                        <div className="status-filter-dropdown d-flex flex-row align-items-center gap-1 card-status-filter">
                          <label>Status : &nbsp;</label>
                          <select
                            className={`p-1 selection ${task.status === 'completed' ? 'bg-success text-white border-success' : 'bg-danger text-white border-danger'}`}
                            value={task.status}
                            onChange={(e) => 
